...
public class RobotContainer {
    ...
    public RobotContainer() {
        ...
        NamedCommands.registerCommand("outtake2", vision.setPriorityId(22, 9).andThen(driveToTargetAuto(ReefSides.LEFT).andThen(elevatorStructure.autoOuttakeCoralCommand())).andThen(Commands.waitSeconds(.1)));
...
//    }
    ...
    public boolean isAtPositionAuto(ReefSides side) {
        ...
        if(Math.abs(errorPose.getX()) < .6 && Math.abs(errorPose.getY()) < .6 && Math.abs(errorPose.getRotation().getDegrees()) < 6){
            return true;
        }
        else{
            return false;
